...
export default function MoviesPage() {
  ...
  const BookmarkingCHeckking = (movie: any) => {
    const bookmarkMovie = BookmarkedMovies.find((item) => {
      return item.title === movie.title;
    });
    if (bookmarkMovie) {
      const filteredMovie = BookmarkedMovies.filter((item) => {
        return item.title !== bookmarkMovie.title;
      });
      setBookmarkedMovies(filteredMovie);
    } else {
  ...
  return (
    ...
    <NavBar />
      <main className="lg:ml-[160px]">
            ...
            <div className="slider w-full h-fit flex flex-row overflow-scroll gap-4">
              {DataMovies.slice(0, 5).map((item, index) => (
                <div key={index} className="max-xs:w-full">
                  <div
                    className="w-[240px] h-[140px] bg-center flex items-center justify-between flex-row rounded-[10px] bg-cover bg-[linear-gradient(180deg,_rgba(0,_0,_0,_0.00)_0%,_rgba(0,_0,_0,_0.75)_100%)] md:w-[470px] md:h-[230px]"
                    style={{
                      backgroundImage: `url(${item.thumbnail.trending?.large})`,
                    }}>
                    <div className="px-4 mt-[90px] flex flex-col gap-1 md:mt-[155px]">
        ...
        <div className="w-full flex flex-row justify-center gap-3 flex-wrap px-4 md:justify-center">
          {finallMovies.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="bg max-xs:w-full relative max-md:w-full">
                <div
                  ...
                  <svg
                    onClick={() => BookmarkingCHeckking(item)}
