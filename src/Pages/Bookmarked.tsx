...
export default function Movies() {
  ...
  const TvSeriesBookMarked = DataMoviesCopy.filter(
    (item) => item.isBookmarked === true && item.category === "TV Series"
  );
  const [Booksmarked, setBooksmarked] = useState<[]>([]);
  const ToggleMarked = (movie) => {
    const bookmarkedMovie = Booksmarked.find((item) => {
      return item.title === movie.tittle;
    });
    if (bookmarkedMovie) {
      const filteredBoookMarkedMovie = Booksmarked.filter((item) => {
        return item.title !== bookmarkedMovie.title;
      });
      setBooksmarked(filteredBoookMarkedMovie);
    } else {
      setBooksmarked((prev) => {
        return [...prev, movie];
      });
      ...
      <main>
        <section>
          <div className="w-full flex flex-col justify-center gap-3 flex-wrap px-4">
            ...
            <div className="w-full flex flex-row justify-center gap-3 flex-wrap">
              {TvSeriesBookMarked.map((item, index) => (
                <div key={index} className="bg max-xs:w-full relative">
                  <div
                    className="w-[240px] h-[140px] bg-center flex flex-wrap rounded-[10px] bg-cover bg-[linear-gradient(180deg,_rgba(0,_0,_0,_0.00)_0%,_rgba(0,_0,_0,_0.75)_100%)] max-md:w-[220px] max-sm:w-[156px] max-sm:h-[100px] max-xs:w-full"
                    style={{
